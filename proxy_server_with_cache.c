...
int connectRemoteServer(char* host_addr, int port_num)
{
	...
	if(host==NULL)
	{
		fprintf(stder, "No such host exist\n");
		return -1;
	}
...
int handle_request(int clientSocketId, ParserRequest *request, char* tempReq)
{
	...
	if(ParsedHeader_get(request, "Host") == NULL){
		if(ParserHeader_set(request, "Host", request->host)<0){
...
void *thread_fn(void *socketNew)
{
	...
	if(temp != NULL)
	{
		int size = temp->len(size)/sizeof(char);
		int pos = 0;
	...
	{
		len = strlen(buffer);
		ParserRequest *request = ParserRequest_create();
		...
		ParseRequest_destroy(request);
	}
	else if(bytes_send_clinet == 0)
	{
...
int main(int argc, char* argv[]){
	...
	while(1)
	{
		...
		struct sockaddr_in * client_pt = (struct socketaddr_in*)&client_addr;
		...
		pthread_create();
		pthread_create(&id[i], NULL, thread_fn, (void *),&Connected_socketId[i]);
		i++;
	}
...
cache_element *find(char* url)
{
	...
	printf("Remove Cache Lock acquire %d\n", temp_lock_val)
	if(head!=NULL)
	{
		site = head;
		while(site != NULL)
